...
namespace bccallcenter\modules\tracking\controllers;
...
use bc\modules\transaction\models\summary\BcTrackingBcDateRangeSearch;
...
class BcController extends Controller {
    ...
    public function behaviors() {
        return [
            'access' => [
                ...
                'only' => ['transaction', 'detailgraph', 'view', 'overallperformance', 'monthlyperformance', 'weeklyperformance', 'dailyperformance', 'downloadoverall'],
                'rules' => [
                    [
                        'actions' => ['transaction', 'detailgraph', 'overallperformance', 'monthlyperformance', 'weeklyperformance', 'dailyperformance', 'downloadoverall'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
    ...
    public function actionTransaction() {
        ...
        try {
            ...
            $searchModels = new BcTrackingBcDateRangeSearch();
    ...
    public function actionDetailgraph() {
        ...
        $model = BcTrackingBcDateRange::findOne($_REQUEST['expandRowKey']);
        
        $graph = new Graph();
//        $bc_trans = $graph->getTimetransactionbc(Graph::TYPE_NO_OF_TRANSACTION, $model);
//        $bc_com = $graph->getTimetransactionbc(Graph::TYPE_TRANSACTION_COM, $model);
        $bc_graph = $graph->getTrans(Graph::TYPE_TRANSACTION_COM, $model);
        $bc_month_graph = $graph->getTransmonthly(Graph::TYPE_TRANSACTION_COM, $model);

        return $this->renderAjax('detailgraph', [
                    'graph'=>$graph,
                    'models' => $model,
                    'bc_graph' => $bc_graph,
                    'bc_month_graph' => $bc_month_graph,
        ]);
    }

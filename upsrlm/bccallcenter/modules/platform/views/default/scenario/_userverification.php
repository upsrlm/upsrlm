...
<div class="row">
    ...
    <div class="col-md-12 form-group">
        <?= Html::label('समूह का नाम, पदाधिकारीयों के नाम व फ़ोन नम्बर सत्यापित करें', 'shg_name_and_other_verify', ['class' => 'label shg_name_and_other_verify font-weight-bold']) ?>
    ...
    <div class="col-md-12 form-group smart_phone">
        ...
        <?= Html::radioList($scenario_form . '[smart_phone]', [], [1 => 'स्वयं का स्मार्ट फोन', 3 => 'परिवार के सदस्य', 2 => 'कोई भी नहीं']) ?>
    </div>

    <div class="col-md-12 form-group userhavemobile">
        <?= Html::label('Do you agree to download Rishta app in your phone', 'agree_download_rishta_app', ['class' => 'label agree_download_rishta_app font-weight-bold']) ?>
        <?= Html::radioList($scenario_form . '[agree_download_rishta_app]', [], [1 => 'Yes', 2 => 'No']) ?>
    </div>

    <div class="col-md-12 form-group else_having_smart_phone">
        <?= Html::label('Who else is having smart phone in SHG', 'else_having_smart_phone', ['class' => 'label else_having_smart_phone font-weight-bold']) ?>
        <?= Html::radioList($scenario_form . '[else_having_smart_phone]', [], $else_having_smart_phone_option) ?>
    </div>

    <div class="col-md-12 form-group carry_smart_phone">
        <?= Html::label('Do they carry the smartphone with them', 'carry_smart_phone', ['class' => 'label carry_smart_phone font-weight-bold']) ?>
        <?= Html::radioList($scenario_form . '[carry_smart_phone]', [], [1 => 'Yes', 2 => 'No']) ?>
    </div>
...
$js = <<<JS
    $(document).ready(function() {
        ...
        $(".carry_smart_phone").css("display","none");
        ...
        if($('input[type=radio][name="{$scenario_form}[else_having_smart_phone]"]:checked').val() === '1'){
        ...
        $('input[type=radio][name="{$scenario_form}[shg_name_and_other_verify]"]').change(function() {
            if($(this).val() =='1'){
                ...
                $('input[name="{$scenario_form}[else_having_smart_phone]"]').prop('checked', false);
                $('input[name="{$scenario_form}[carry_smart_phone]"]').prop('checked', false);
        ...
        $('input[type=radio][name="{$scenario_form}[smart_phone]"]').change(function() {
            if($(this).val() =='1'){
                $(".userhavemobile").css("display","block");
                $(".carry_smart_phone").css("display","block");
                $(".else_having_smart_phone").css("display","none");
                $('input[name="{$scenario_form}[else_having_smart_phone]"]').prop('checked', false);
                ...
                if($(this).val() =='2'){
                    $(".else_having_smart_phone").css("display","block");
                }else{
                    $(".else_having_smart_phone").css("display","none");
                }
                $(".userhavemobile").css("display","none");
        ...
        $('input[type=radio][name="{$scenario_form}[else_having_smart_phone]"]').change(function() {
            if($(this).val() !='0'){
                $(".carry_smart_phone").css("display","block");
            }else{
                $(".carry_smart_phone").css("display","none");

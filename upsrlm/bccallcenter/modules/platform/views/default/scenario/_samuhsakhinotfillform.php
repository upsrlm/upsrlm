...
use common\models\dynamicdb\internalcallcenter\platform\CallingScenarioSamuhsakhinotfillform;
        ...
        <div class="row">
            <div class="col-md-9">
                <div class="mb-3 form-group">
                    <?= Html::label('Samuh Sakhi Not Fill Form in Rishta APP Ask Why?', 'samuh_sakhi_not_fillform', ['class' => 'label samuh_sakhi_not_fillform font-weight-bold']) ?>
                    <?= Html::radioList($scenario_form . '[samuh_sakhi_not_fillform]', [], CallingScenarioSamuhsakhinotfillform::samuhsakhinotfillformoption()) ?>
                </div>
...
$js = <<<JS
    $(document).ready(function() {
        ...
        if($('input[type=radio][name="CallingListForm[call_status]"]:checked').val()=='10'){
            $('input[name="{$scenario_form}[samuh_sakhi_not_fillform]"]').prop('required', true);
            $('input[name="scenario_form[call_purpose_complete]"]').prop('required', true);
        }  


        if($('input[type=radio][name="{$scenario_form}[samuh_sakhi_not_fillform]"]:checked').val() === '2'){
        ...
        $('input[type=radio][name="{$scenario_form}[samuh_sakhi_not_fillform]"]').change(function() {
            if($(this).val() =='2'){
                $(".menunotshowing").css("display","block");
            }else{
                $(".menunotshowing").css("display","none");
        ...
        function uncheckedinternalfomrelement(){
            $('input[name="scenario_form[samuh_sakhi_not_fillform]"]').prop('checked', false);
            ...
            $('input[name="{$scenario_form}[samuh_sakhi_not_fillform]"]').prop('required', false);

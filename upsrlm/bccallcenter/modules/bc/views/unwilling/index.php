                        ...
                        'columns' => [
                            ...
                            [
                                ...
                                'value' => function ($model) {
                                    ...
                                    return $model->name . '<br/>' . $model->guardian_name . $html;
                                },
                            ],
//                            [
...
//                            ],
                            ...
                            [
                                ...
                                'format' => 'raw',
                                'header' => 'Unwilling Reason by RSETI',
                                'value' => function ($model) {
                            ...
                            [
                                'attribute' => 'Action',
                                'format' => 'raw',
                                'visible' => isset(Yii::$app->user->identity->role) and in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_DBT_CALL_CENTER_EXECUTIVE, MasterRole::ROLE_DBT_CALL_CENTER_MANAGER, MasterRole::ROLE_INTERNAL_CALL_CENTER_ADMIN, MasterRole::ROLE_INTERNAL_CALL_CENTER_EXECUTIVE]),
                                'contentOptions' => ['style' => 'width: 5%'],
                                'value' => function ($model) {
                                    $html = '<span id="' . $model->id . '">';

                                    if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_DBT_CALL_CENTER_EXECUTIVE, MasterRole::ROLE_DBT_CALL_CENTER_MANAGER, MasterRole::ROLE_INTERNAL_CALL_CENTER_ADMIN, MasterRole::ROLE_INTERNAL_CALL_CENTER_EXECUTIVE])) {
                                        if (in_array($model->training_status, [0])) {
                                            if ($model->bc_unwilling_rsetis == 1 and $model->bc_unwilling_call_center != 1 and $model->bc_unwilling_call_center != '0') {
                                                $html .= yii\helpers\Html::button('Unwilling/Unreachable', ['id' => 'take-unwilling-' . $model->id, 'class' => 'btn btn-sm btn-block btn-info btn-block popb', 'value' => '/bc/unwilling/call?id=' . $model->id, 'name' => 'takeaction', 'title' => 'Unwilling/Unreachable']) . '<br/>';
                                            }
                                        }
                                    }
                            ...
                            [
                                ...
                                'value' => function ($model) {
                                    ...
                                    if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_DBT_CALL_CENTER_EXECUTIVE, MasterRole::ROLE_DBT_CALL_CENTER_MANAGER, MasterRole::ROLE_INTERNAL_CALL_CENTER_ADMIN, MasterRole::ROLE_INTERNAL_CALL_CENTER_EXECUTIVE])) {
                                        if (in_array($model->training_status, [0])) {
                                            if ($model->bc_unwilling_rsetis == 1 and $model->bc_unwilling_call_center != 1 and $model->bc_unwilling_call_center != '0') {
                                                $html .= \yii\helpers\Html::a('<i class="fal fa-phone"></i>कॉल करें', ['/platform/call/scenarioatend?bcid=' . $model->id . '&scenario=517'], ['target' => '_blank', 'data-pjax' => 0, 'class' => 'btn btn-info']);
                                            }
                                        }
                                    }
                                    return $html;

...
<div class='profile-form' style="padding:0px 10px;background-color: white">
    <?php
    if (isset($model->profle_model))
        echo $this->render('verificationstaus', ['model' => $model->profle_model]);
    ?>
    <div class="row">
    ...
    <div class="form-group">
        ...
        <div class="col-md-3">  
            <?= Html::a('<i class="fa fa-thumb-up"></i> Update Profile', '/profile/update', ['id' => 'view-button', 'class' => 'btn btn-sm btn-info btn-block', 'style' => 'width:300px', 'title' => '']); ?>
        </div>       
        <?php {
            if ($model->user_id == \Yii::$app->user->identity->id && $model->profle_model->is_profile_complete != "1") {
                ?>
                <div class="col-md-6">
                    <?= Html::submitButton('Submit the profile', ['class' => 'btn btn-success']) ?></div>
                <?php
            }
        }
        ?>

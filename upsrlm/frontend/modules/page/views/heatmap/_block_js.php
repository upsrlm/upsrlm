...
$script = <<< JS
    $(document).ready(function () {
        ...
        var boundary1 = new L.geoJson(up_geojson, { style: assignColor, onEachFeature: onEachFeatureCurrent }).addTo($map_current_data);
           
    
        $.each(gp_list, function(code, gp_data) {
            let gp_json_name = 'gp_' + gp_data.id + '_geojson';
            let geojsonData = window[gp_json_name]; 
            var boundary_gp = new L.geoJson(geojsonData, { style: assignColor, onEachFeature: onEachFeatureCurrent })
        ...
        function onEachFeatureCurrent(feature, layer) {
            ...
            layer.on('click', function () {
                $('#analyticsheatmap-gram_panchayat_code').val(feature.properties.gram_panchayat_code).trigger("change");
                $('form').submit();
            });
        }

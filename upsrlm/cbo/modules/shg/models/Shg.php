...
namespace app\modules\shg\models;
 ...
 */
class Shg extends \common\models\dynamicdb\cbo\CboactiveRecord {
    ...
    public function rules() {
        return [
            ...
            [['verify_chaire_person_mobile_no', 'verify_secretary', 'verify_treasurer', 'verify_over_all', 'verify_ques1', 'verify_ques2', 'verify_ques3', 'verify_shg_member_by'], 'integer'],
    ...
    public function beforeSave($insert) {
        ...
        if ($this->bank_id) {
            $bank_model = \cbo\models\master\CboMasterBank::findOne($this->bank_id);
            if (!empty($bank_model)) {
    ...
    public function getRprofile() {
        return $this->hasOne(\cbo\models\sakhi\RishtaShgProfile::className(), ['cbo_shg_id' => 'id']);
    }

    public function getRfeedback() {
        return $this->hasOne(\cbo\models\sakhi\RishtaShgFeedback::className(), ['cbo_shg_id' => 'id']);
    }

    public function getRmembers() {
        return $this->hasMany(\cbo\models\sakhi\RishtaShgMember::className(), ['cbo_shg_id' => 'id'])->where([RishtaShgMember::getTableSchema()->fullName . '.status' => 1]);
    }

    public function getRfundrecived() {
        return $this->hasMany(\cbo\models\sakhi\RishtaShgFundReceived::className(), ['cbo_shg_id' => 'id'])->where([RishtaShgFundReceived::getTableSchema()->fullName . '.status' => 1]);
    }
    ...
    public function getDupac() {
        $model = Shg::find()->andWhere(['dummy_column' => 0])->andWhere(['not', ['bank_account_no_of_the_shg' => null]])->andWhere(['bank_account_no_of_the_shg' => $this->bank_account_no_of_the_shg])->all();
        return $model;
    }

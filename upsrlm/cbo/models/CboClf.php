 ...
 * @property int $status
 */
class CboClf extends \common\models\dynamicdb\cbo_detail\CboDetailactiveRecord {
    ...
    public function beforeSave($insert) {
        if ($this->block_code != NULL) {
            $block_model = \common\models\master\MasterBlock::find()->where(['block_code' => $this->block_code])->one();
            if (!empty($block_model)) {
                $this->division_code = $block_model->division_code;
                $this->division_name = $block_model->division_name;
                $this->district_code = $block_model->district_code;
                $this->district_name = $block_model->district_name;
                $this->block_code = $block_model->block_code;
                $this->block_name = $block_model->block_name;
            }
        }
        if ($this->bank_id) {
        ...
        if ($this->bank_id2) {
            $bank_model = master\CboMasterBank::findOne($this->bank_id2);
            if (!empty($bank_model)) {
                $this->name_of_bank2 = $bank_model->bank_name;
            }
        }
        if ($this->bank_id3) {
            $bank_model = master\CboMasterBank::findOne($this->bank_id3);
            if (!empty($bank_model)) {
                $this->name_of_bank3 = $bank_model->bank_name;
            }
        }
        ...
        if ($this->date_of_opening_the_bank_account2 != null and $this->date_of_opening_the_bank_account2 != '') {
            $this->date_of_opening_the_bank_account2 = \Yii::$app->formatter->asDatetime($this->date_of_opening_the_bank_account2, "php:Y-m-d");
        }
        if ($this->date_of_opening_the_bank_account3 != null and $this->date_of_opening_the_bank_account3 != '') {
            $this->date_of_opening_the_bank_account3 = \Yii::$app->formatter->asDatetime($this->date_of_opening_the_bank_account3, "php:Y-m-d");
        }
        if ($this->updated_balance_in_bank_date != null and $this->updated_balance_in_bank_date != '') {
            $this->updated_balance_in_bank_date = \Yii::$app->formatter->asDatetime($this->updated_balance_in_bank_date, "php:Y-m-d");
        }
        if ($this->updated_balance_in_bank_date2 != null and $this->updated_balance_in_bank_date2 != '') {
            $this->updated_balance_in_bank_date2 = \Yii::$app->formatter->asDatetime($this->updated_balance_in_bank_date2, "php:Y-m-d");
        }
        if ($this->updated_balance_in_bank_date3 != null and $this->updated_balance_in_bank_date3 != '') {
            $this->updated_balance_in_bank_date3 = \Yii::$app->formatter->asDatetime($this->updated_balance_in_bank_date3, "php:Y-m-d");
        }
        $this->bank_balance = (float) ((float) $this->updated_balance_in_bank + (float) $this->updated_balance_in_bank2 + (float) $this->updated_balance_in_bank3);
        if (!$this->isNewRecord) {
            $total_funds = CboClfFunds::find()->where(['cbo_clf_id' => $this->id, 'status' => 1])->sum('total_amount_received');
            $startup_funds = CboClfFunds::find()->where(['cbo_clf_id' => $this->id, 'fund_type' => self::FUNDS_TYPE_START_UP_ID, 'status' => 1])->sum('total_amount_received');
            $cif_funds = CboClfFunds::find()->where(['cbo_clf_id' => $this->id, 'fund_type' => self::FUNDS_TYPE_CIF_ID, 'status' => 1])->sum('total_amount_received');
            $isf_funds = CboClfFunds::find()->where(['cbo_clf_id' => $this->id, 'fund_type' => self::FUNDS_TYPE_ISF_ID, 'status' => 1])->sum('total_amount_received');
            $infra_funds = CboClfFunds::find()->where(['cbo_clf_id' => $this->id, 'fund_type' => self::FUNDS_TYPE_IF_ID, 'status' => 1])->sum('total_amount_received');
            $vo_funds = CboClfFunds::find()->where(['cbo_clf_id' => $this->id, 'fund_type' => self::FUNDS_TYPE_VO, 'status' => 1])->sum('total_amount_received');
            $other_funds = CboClfFunds::find()->where(['cbo_clf_id' => $this->id, 'fund_type' => self::FUNDS_TYPE_OTHERS_ID, 'status' => 1])->sum('total_amount_received');
            $this->total_funds_amount = $total_funds != null ? $total_funds : 0;
            $this->start_up_funds_amount = $startup_funds != null ? $startup_funds : 0;
            $this->cif_funds_amount = $cif_funds != null ? $cif_funds : 0;
            $this->isf_funds_amount = $isf_funds != null ? $isf_funds : 0;
            $this->if_funds_amount = $infra_funds != null ? $infra_funds : 0;
            $this->other_funds_amount = $other_funds != null ? $other_funds : 0;
            $this->vo_funds_amount = $vo_funds != null ? $vo_funds : 0;
        }
    ...
    public function getFeedback() {
        return $this->hasOne(CboClfFeedback::className(), ['cbo_clf_id' => 'id']);
    }

    public function getMembers() {
        return $this->hasMany(CboClfMembers::className(), ['cbo_clf_id' => 'id'])->where(['status' => 1]);
    }

    public function getFunds() {
        return $this->hasMany(CboClfFunds::className(), ['cbo_clf_id' => 'id'])->where(['status' => 1, 'received_by' => CboClfFunds::RECEIVED_BY_GOV]);
    }

    public function getFundsfindvo() {
        return $this->hasMany(CboClfFunds::className(), ['cbo_clf_id' => 'id'])->where(['status' => 1, 'received_by' => CboClfFunds::RECEIVED_BY_VO]);
    }

    public function getVos() {
        return $this->hasMany(CboVo::className(), ['cbo_clf_id' => 'id']);
    }
    ...
    public function getPassbookUrl() {
        ...
        return Yii::$app->params['app_url']['hr'] . "/getimage/cbo/clf/" . $this->id . "/" . $this->passbook_photo;
    }

    public function getPassbook2Url() {

        return Yii::$app->params['app_url']['hr'] . "/getimage/cbo/clf/" . $this->id . "/" . $this->passbook_photo2;
    }
    public function getPassbook3Url() {

        return Yii::$app->params['app_url']['hr'] . "/getimage/cbo/clf/" . $this->id . "/" . $this->passbook_photo3;
    }
    public function getPanphotoUrl() {

        return Yii::$app->params['app_url']['hr'] . "/getimage/cbo/clf/" . $this->id . "/" . $this->pan_photo;
    }
    ...
    public function getRishtaclf() {
        return $this->hasOne(\common\models\dynamicdb\cbo_detail\RishtaClf::className(), ['id' => 'id']);
    }
    public function afterSave($insert, $changedAttributes) {
        $attribute = CboClf::findOne($this->id);
        try {

            $cbo_detail = new \common\models\dynamicdb\cbo_detail\RishtaClf();
            $modelcbod = $cbo_detail::findOne($attribute->id);

            if (empty($modelcbod)) {
                $modelcbod = new \common\models\dynamicdb\cbo_detail\RishtaClf();
            }

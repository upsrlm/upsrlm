...
/**
 * This is the model class for table "dbt_beneficiary_scheme_bocw_payment".
 *
 * @property int $id
 * @property int|null $dbt_beneficiary_scheme_bocw_labour_scheme_status_id
 * @property string|null $uplmis_app_no
 * @property string|null $uplmis_lab_reg_no
 * @property string|null $uplmis_app_date
 * @property string|null $uplmis_payment_date
 * @property string|null $uplmis_scheme_name
 * @property string|null $uplmis_payment_amt
 ...
 * @property int $status
 */
class DbtBeneficiarySchemeBocwPayment extends CboDetailactiveRecord {

    /**
     * {@inheritdoc}
     */
    public static function tableName() {
        return 'dbt_beneficiary_scheme_bocw_payment';
    }
    ...
    public function rules() {
        return [
            [['uplmis_gram_panchayat_code', 'scheme_id', 'division_code', 'district_code', 'block_code', 'gram_panchayat_code', 'created_at', 'updated_at', 'status','dbt_beneficiary_scheme_bocw_labour_scheme_status_id'], 'integer'],
            [['app_date', 'payment_date', 'dob'], 'safe'],
            [['payment_amt'], 'number'],
            [['uplmis_app_no', 'uplmis_lab_reg_no', 'uplmis_app_date', 'uplmis_payment_date'], 'string', 'max' => 50],
            [['uplmis_scheme_name', 'uplmis_temp_address'], 'string', 'max' => 255],
            [['uplmis_payment_amt'], 'string', 'max' => 1000],
    ...
    public function attributeLabels() {
        return [
            ...
            'uplmis_payment_date' => 'Uplmis Payment Date',
            'uplmis_scheme_name' => 'Uplmis Scheme Name',
            'uplmis_payment_amt' => 'Uplmis Payment Amt',
    ...
    public function beforeSave($insert) {
        if ($this->uplmis_app_date) {
            $this->app_date = date('Y-m-d H:i:s', strtotime($this->uplmis_app_date));
        }
        if ($this->uplmis_payment_date) {
            $this->payment_date = date('Y-m-d H:i:s', strtotime($this->uplmis_payment_date));
        }
        if ($this->uplmis_dob) {
            $this->dob = date('Y-m-d H:i:s', strtotime($this->uplmis_dob));
        }
        if ($this->uplmis_payment_amt) {
            $this->payment_amt = $this->uplmis_payment_amt;
        }
        $scheme_model = \common\models\dynamicdb\cbo_detail\dbt\DbtBeneficiarySchemeBocwMasterBenifits::find()->andFilterWhere(['like', 'name_hi', $this->uplmis_scheme_name])->one();
        if ($scheme_model != null) {
            $this->scheme_id = $scheme_model->id;
        }

        return parent::beforeSave($insert);
    }
    public function getSchemeapplication() {
        return $this->hasOne(DbtBeneficiarySchemeBocwLabourSchemeStatus::className(), ['id' => 'dbt_beneficiary_scheme_bocw_labour_scheme_status_id']);
    }

    public function getLabour() {
        return $this->hasOne(DbtBeneficiarySchemeBocwLabourProfile::className(), ['id' => 'dbt_beneficiary_scheme_bocw_labour_profile_id'])->via('schemeapplication');
    }
}

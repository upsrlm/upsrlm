 ...
 * @property int $status
 */
class RelationUserBdoBlock extends \common\models\dynamicdb\bc\BcactiveRecord {
    ...
    public function afterSave($insert, $changedAttributes) {
        $attribute = $this;
        try {
                $bc = new \common\models\dynamicdb\bc\RelationUserBdoBlock();
            $modelbc = $bc::findOne($attribute->id);

            if (empty($modelbc)) {
                $modelbc = new \common\models\dynamicdb\bc\RelationUserBdoBlock();
            }
            $modelbc->setAttributes($attribute->attributes);
            $modelbc->id = $attribute->id;
            $modelbc->user_id = $attribute->user_id;
            $modelbc->master_block_id = $attribute->master_block_id;

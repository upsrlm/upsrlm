...
$this->title = 'Frontier Market Admin/ Frontier Market District Admin';
                            ...
                            [
                                ...
                                'value' => function ($model) {
                                    ...
                                    if (in_array($model->role, [MasterRole::ROLE_CALL_CENTER_EXECUTIVE])) {
                                        ...
                                        $html .= implode(', ', array_unique(ArrayHelper::getColumn($model->districts, 'district.district_name')));
                                    } elseif (in_array($model->role, [MasterRole::ROLE_FRONTIER_MARKET_DISTRICT_ADMIN])) {
                                        $html .= implode(', ', array_unique(ArrayHelper::getColumn($model->districts, 'district.district_name')));
...
//                        'contentOptions' => ['style' => 'width: 18%'],
                                'value' => function ($model) {
                                    ...
                                    if (in_array($model->role, [MasterRole::ROLE_FRONTIER_MARKET_DISTRICT_ADMIN])) {
                                        $html .= Html::a('<i class="fal fa fa-edit"></i>', ['/user/updatefmda', 'userid' => $model->id], [
                                                    'title' => 'Frontier Market District Admin',
                                                    'data-pjax' => "0",
                                                    'class' => 'btn btn-sm btn-info mb-2',
                                                ]) . ' ';
                                    }
                                    if (in_array($model->role, [MasterRole::ROLE_FRONTIER_MARKET_ADMIN])) {

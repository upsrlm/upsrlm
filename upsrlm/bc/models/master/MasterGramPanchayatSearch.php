...
class MasterGramPanchayatSearch extends MasterGramPanchayat {
    ...
    public $fourth_complete;
    public $fifth_complete;
    public $fifth_vacant;
    public $six_complete;
    public $six_vacant;
    public $seventh_complete;
    public $seventh_vacant;
    public $aspirational;
    public $igrs;
    /**
     * {@inheritdoc}
    ...
    public function rules() {
        return [
            ...
            [['previous_master_partner_bank_id'], 'safe'],
            [['saheli'], 'safe'],
            [['second_complete'], 'safe'],
            [['third_complete'], 'safe'],
            [['fourth_complete'], 'safe'],
            [['fifth_vacant'], 'safe'],
            [['fifth_complete'], 'safe'],
            [['aspirational'], 'safe'],
            [['six_vacant'], 'safe'],
    ...
    public function search($params, $user_model = null, $pagination = true, $distinct_column = null, $columns = null, $array = false) {
        ...
        if ($user_model == NULL) {
            ...
            if (in_array($user_model->role, [MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_ADMIN, MasterRole::ROLE_CALL_CENTER_ADMIN])) {
                ...
                $query->andWhere([MasterGramPanchayat::getTableSchema()->fullName . '.master_partner_bank_id' => $user_model->master_partner_bank_id]);
                $query->andWhere([MasterGramPanchayat::getTableSchema()->fullName . '.district_code' => \yii\helpers\ArrayHelper::getColumn($user_model->districts, 'district_code')]);
        ...
        if ($this->igrs) {
            $query->andWhere([MasterGramPanchayat::getTableSchema()->fullName . '.district_code' => \Yii::$app->params['igrs_disricts']]);
        }
...
// add conditions that should always apply here
        ...
        $query->andFilterWhere([
            ...
            'master_gram_panchayat_detail_bc.fifth_complete' => $this->fifth_complete,
            'master_gram_panchayat.bc_status' => $this->bc_status,
            'master_gram_panchayat.master_partner_bank_id' => $this->master_partner_bank_id,
            'master_gram_panchayat.previous_master_partner_bank_id' => $this->previous_master_partner_bank_id,
            'master_gram_panchayat.previous_master_partner_bank_id' => $this->previous_master_partner_bank_id,
        ]);
    ...
    public function searchopenapplcation($params, $user_model = null, $pagination = true, $distinct_column = null, $columns = null, $array = false) {
        ...
        $query->andWhere(['master_gram_panchayat.status' => 1]);
        $query->andWhere(['master_gram_panchayat_detail_bc.seventh_vacant' => 1]);
    ...
    public function searchin($params, $user_model = null, $pagination = true, $distinct_column = null, $columns = null, $array = false) {
        if (isset($params->attributes))
            $this->setAttributes($params->attributes);
        else {
            $this->load($params);
        ...
        $query->joinWith(['gpdetail']);
        $query->andWhere(['master_gram_panchayat.status' => 1]);
        $query->andWhere(['>=', 'master_gram_panchayat.change_status', 1]);
        $query->andWhere(['!=', 'master_gram_panchayat.district_code', 0]);
        $query->andWhere(['!=', 'master_gram_panchayat.block_code', 0]);
        if ($user_model == NULL) {
            $query->where('0=1');
...
// add conditions that should always apply here
    ...
    public function searchout($params, $user_model = null, $pagination = true, $distinct_column = null, $columns = null, $array = false) {
        if (isset($params->attributes))
            $this->setAttributes($params->attributes);
        else {
            $this->load($params);
        ...
        if ($user_model == NULL) {
            ...
            if (in_array($user_model->role, [MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_ADMIN, MasterRole::ROLE_CALL_CENTER_ADMIN])) {
                ...
                $query->andWhere([MasterGramPanchayat::getTableSchema()->fullName . '.previous_master_partner_bank_id' => $user_model->master_partner_bank_id]);
            } elseif (in_array($user_model->role, [MasterRole::ROLE_BANK_FI_PARTNER_DISTRICT_NODAL,MasterRole::ROLE_CORPORATE_BCS])) {
                $query->andWhere([MasterGramPanchayat::getTableSchema()->fullName . '.previous_master_partner_bank_id' => $user_model->master_partner_bank_id]);
            } else {
                $query->where('0=1');
...
// add conditions that should always apply here
        ...
        $query->andFilterWhere([
            ...
            'master_gram_panchayat_detail_bc.fifth_complete' => $this->fifth_complete,
            'master_gram_panchayat.bc_status' => $this->bc_status,
            'master_gram_panchayat.change_status' => $this->change_status,
        ]);

...
class MasterBlockSearch extends MasterBlock {
    ...
    public function rules() {
        return [
            [['id', 'division_code', 'district_code', 'sub_district_code', 'gram_panchayat_count', 'village_count', 'nretp'], 'safe'],
            [['division_name', 'district_name', 'sub_district_name', 'block_code', 'block_name', 'aspirational', 'igrs'], 'safe'],
            [['igrs'], 'safe']
        ];
    }
    ...
    public function search($params, $user_model = null, $pagination = true, $columns = null, $array = false) {
        ...
        $query->andWhere([MasterBlock::getTableSchema()->fullName . '.status' => 1]);
        if ($columns != NULL) {
        ...
        if ($this->igrs) {
            $query->andWhere([MasterBlock::getTableSchema()->fullName . '.district_code' => \Yii::$app->params['igrs_disricts']]);
        }

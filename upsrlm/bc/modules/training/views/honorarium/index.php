        ...
        <div id="panel-1" class="panel">
            <div class="panel-hdr">
                ...
                <div class="panel-toolbar">
                    <?php if (isset(Yii::$app->user->identity->role) and in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_SPM_FINANCE])) { ?>
                        <?= Html::a('Upload Honorarium payment CSV', ['upload'], ['class' => 'btn btn-success']) ?>
                    <?php } ?>
...
<!--                                    <i class="fal fa-check position-absolute pos-right pos-bottom opacity-15 mb-n1 mr-n1" style="font-size:6rem"></i>-->
                    ...
                    GridView::widget([
                            ...
                            'columns' => [
                                ...
                                [
                                    ...
                                    'value' => function ($model) {
                                        ...
                                        return $model->bc->name. ' / ' . common\helpers\Utility::mask($model->bc->mobile_no)  . $html . '<br/>' . $model->bc->district_name . ' /' . $model->bc->block_name . ' /' . $model->bc->gram_panchayat_name;
                                    }
                                ],
                                [
                                    'attribute' => 'month1',
                            ...
                            [
                                ...
                                'header' => 'Action',
                                'visible' => in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_SPM_FINANCE]),
                                'template' => '{bcpayment}',
                                'buttons' => [
                                    'bcpayment' => function ($url, $model) {
                                        return ($model->bc->training_status == SrlmBcApplication::TRAINING_STATUS_PASS and $model->bc->urban_shg == '0' and $model->bc->blocked == '0' and in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_SPM_FINANCE])) ? Html::button('Payment', [
                                            'data-pjax' => "0",
                                            'class' => 'btn   btn-info popb',
                                            'value' => '/training/honorarium/payment?bcid=' . $model->bc->id,

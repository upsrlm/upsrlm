...
$this->title = 'Certified BC SHG GP change';
                    ...
                    GridView::widget([
                        ...
                        'columns' => [
                            ...
                            [
                                ...
                                'format' => 'raw',
                                'value' => function ($model) {
                                    return \bc\models\transaction\BcTransaction::find()->where(['bc_application_id'=>$model->id])->exists() ? 'Yes' : 'No';
                                }
                            ],
                            [
...
//
//                                    if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_MD, MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_RSETIS_STATE_UNIT, MasterRole::ROLE_RSETIS_DISTRICT_UNIT])) {
//                                        if ($model->blocked == 9) {
//                                            $html .= yii\helpers\Html::button('<i class="fa fa-thumb-up"></i> Aadhar Duplicacy', ['id' => 'take-already-' . $model->id, 'style' => 'margin-top:5px', 'class' => 'btn  btn-block btn-danger btn-block popb', 'value' => '/training/preselected/aadharduplicacy?bcid=' . $model->id, 'name' => 'takeaction', 'title' => 'Aadhar Duplicacy']) . '<br/>';
//                                        }
//                                    }
//
//                                    $html .= "</span>";
...
//                            ],
    ...
    $('form select').on('change', function(){
     $("#Searchform").attr({ "action":"/training/preselected/bcshggpchange"});                        
...
//              $("#Searchform").attr({ "action":"/training/preselected/certifiedblockeddownload"});

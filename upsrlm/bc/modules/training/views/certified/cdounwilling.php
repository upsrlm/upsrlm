...
$this->title = "Certified BC Unwiling (CDO verification pending)";
...
//                        'contentOptions' => ['style' => 'width: 10%'],
                                ...
                                'header' => 'Action',
                                'visible' => in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_CDO]),
                                'template' => '{unwilling}',
                                'buttons' => [
                                    'unwilling' => function ($url, $model) {
                                        return (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_CDO]) and $model->training_status == SrlmBcApplication::TRAINING_STATUS_PASS and $model->bc_unwilling_bank == 1 and $model->bc_unwilling_bc == 1) ? Html::button('<span class="fa "> Confirm Unwillingness</span>', [
                                            'data-pjax' => "0",
                                            'class' => 'btn btn-danger  btn-block popb',
                                            'value' => '/training/participants/cdounwilling?bcid=' . $model->id,
                                            'title' => 'Confirm Unwillingness BC  : ' . $model->name
                                        ]) . ' ' : '';
                                    },        
                                ]
                            ],        

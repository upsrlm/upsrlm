...
use bc\models\BlockBcActiveProgressMonthly;
...
class BlockController extends Controller {
    ...
    public function progress($month) {
        ...
        if (isset($month->id)) {
            $models = MasterBlock::find()->select(['district_code', 'district_name', 'block_code', 'block_name'])->where(['status' => 1])->all();
            foreach ($models as $model) {
                $progress = BlockBcActiveProgressMonthly::findOne(['block_code' => $model->block_code, 'month_id' => $month->id]);
                if ($progress == null) {
                    $progress = new BlockBcActiveProgressMonthly();
                }
                $progress->district_code = $model->district_code;
                $progress->block_code = $model->block_code;
                $progress->month_id = $month->id;
                $bc_active = GramPanchayatBcActiveProgressMonthly::find()->where(['block_code' => $model->block_code, 'month_id' => $month->id])->sum('bc_active');
                $no_of_transaction = GramPanchayatBcActiveProgressMonthly::find()->where(['block_code' => $model->block_code, 'month_id' => $month->id])->sum('no_of_transaction');
                $transaction_amount = GramPanchayatBcActiveProgressMonthly::find()->where(['block_code' => $model->block_code, 'month_id' => $month->id])->sum('transaction_amount');
                $commission_amount = GramPanchayatBcActiveProgressMonthly::find()->where(['block_code' => $model->block_code, 'month_id' => $month->id])->sum('commission_amount');
                if ($bc_active) {

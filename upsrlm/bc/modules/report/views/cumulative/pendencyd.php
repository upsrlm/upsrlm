        ...
        <div id="panel-1" class="niu_panel">
            ...
            <div class="panel-container show">
                <div class="panel-content">
                    ...
                    <?php if (!$searchModel->master_partner_bank_id) { ?>
                        <div class="panel-tag">
                            <span style="color:#478fca !important"><b>8.a : Agree for training , 8.b : Registered (Batch Assigned)</b>I</span>
                        </div>
                    <?php } ?>
                    <?php if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_MD])) { ?>
                        <!--                        <div class="col-xl-12 col-md-12 col-lg-12">
                        <?= \bc\widgets\GpBcSummary::widget(['model' => $searchModel]) ?>
                                                </div>-->
                    <?php } ?>
                    ...
                    <?php if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_SPM_FI_MF, MasterRole::ROLE_SPM_FINANCE, MasterRole::ROLE_DC_NRLM, MasterRole::ROLE_CDO, MasterRole::ROLE_MD, MasterRole::ROLE_MSC, MasterRole::ROLE_BC_VIEWER])) { ?>
                        ...
                        if (!$searchModel->master_partner_bank_id) {
                            echo $this->render('commond', ['dataProvider' => $dataProvider, 'searchModel' => $searchModel]);
                        }
                        if ($searchModel->master_partner_bank_id) {
                            echo $this->render('bankdupsrlm', ['dataProvider' => $dataProviderb, 'master_partner_bank_id' => $searchModel->master_partner_bank_id, 'model' => $searchModel, 'searchModel' => $searchModel]);
                        }
                        ?>
                    ...
                    <?php if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_RSETIS_STATE_UNIT, MasterRole::ROLE_RSETIS_DISTRICT_UNIT, MasterRole::ROLE_RSETIS_NODAL_BANK, MasterRole::ROLE_UPSRLM_RSETI_ANCHOR])) { ?>
                        <?php echo $this->render('rsethisd', ['dataProvider' => $dataProvider, 'searchModel' => $searchModel]); ?>
                    <?php } ?>
                    <?php if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_BANK_DISTRICT_UNIT, MasterRole::ROLE_BANK_FI_PARTNER_DISTRICT_NODAL, MasterRole::ROLE_CORPORATE_BCS])) { ?>
                        <?php echo $this->render('bankd', ['dataProvider' => $dataProviderb, 'searchModel' => $searchModel]); ?>
                    <?php } ?>
                    <?php
//                    $script = <<< JS

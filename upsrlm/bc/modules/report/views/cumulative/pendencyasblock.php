...
$this->title = "Aspirational blocks pendency";
...
<!--                         <button id="exportBtn1" class="btn  btn-info float-right"> <i class="fal fa-file-excel" style="color: red"> </i> Export To Excel</button><br><br>-->
                    ...
                    <?php ?>
                    <?php if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_ADMIN, MasterRole::ROLE_SUPER_ADMIN, MasterRole::ROLE_SPM_FI_MF, MasterRole::ROLE_SPM_FINANCE, MasterRole::ROLE_DC_NRLM, MasterRole::ROLE_CDO, MasterRole::ROLE_MD, MasterRole::ROLE_MD])) { ?>
                        <?php echo $this->render('commondbas', ['dataProvider' => $dataProvider, 'searchModel' => $searchModel]); ?>
                    <?php } ?> 
                    <?php if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_RSETIS_STATE_UNIT, MasterRole::ROLE_RSETIS_DISTRICT_UNIT, MasterRole::ROLE_RSETIS_NODAL_BANK, MasterRole::ROLE_UPSRLM_RSETI_ANCHOR])) { ?>
                        <?php echo $this->render('rsethisdbas', [ 'dataProvider' => $dataProvider, 'searchModel' => $searchModel]); ?>
                    <?php } ?>
                    <?php if (in_array(Yii::$app->user->identity->role, [MasterRole::ROLE_BANK_DISTRICT_UNIT, MasterRole::ROLE_BANK_FI_PARTNER_DISTRICT_NODAL,MasterRole::ROLE_CORPORATE_BCS])) { ?>
                        <?php echo $this->render('bankdbas', ['dataProvider' => $dataProvider, 'searchModel' => $searchModel]); ?>
                    <?php } ?>    
                    <?php
                    $script = <<< JS
    ...
    $(document).ready(function () {
      $("#exportBtn1").click(function(){
        TableToExcel.convert(document.getElementById("report"), {
            name: "ACS' report for daily monitoring - bc_pendency_report_aspirational_blocks.xlsx",
            sheet: {
            name: "Sheet1"
            }
          });

...
class DashboardController extends Controller {
    ...
    public function actionIndex() {
        ...
        $registeruser = \bc\modules\selection\models\SrlmBcSelectionUser::find()->select(['id'])->where(['phase' => 5])->count();
        ...
        $block_option_model = $block_model->orderBy('block_name asc')->all();
        $searchModel->block_option = \yii\helpers\ArrayHelper::map($block_option_model, 'block_code', 'block_name');
        if ($searchModel->block_code) {
            $gp_model = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['master_gram_panchayat.gram_panchayat_code', 'gram_panchayat_name'])->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['or',
                ['master_gram_panchayat_detail_bc.fifth_vacant' => 1],
            ]);

            $gp_model->andWhere(['block_code' => $searchModel->block_code]);

            $gp_option_model = $gp_model->orderBy('gram_panchayat_name asc')->all();
...
//                }
    ...
    public function actionDownload() {
        ...
        $dataProvider = $searchModels->search($searchModel, Yii::$app->user->identity, false, 'srlm_bc_application5.id');
        ...
        foreach ($models as $model) {
            ...
            $row = [
                ...
                $model->form_number,
                $form_start_date,
                SrlmBcApplication5::find()->select('id')->where(['mobile_number' => $model->mobile_number, 'form_number' => 6])->asArray()->count(),
                SrlmBcApplication5::find()->select('id')->where(['aadhar_number' => $model->aadhar_number, 'form_number' => 6])->asArray()->count(),

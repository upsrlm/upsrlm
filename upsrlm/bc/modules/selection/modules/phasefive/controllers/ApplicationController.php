...
namespace bc\modules\selection\modules\phasefive\controllers;
...
use bc\modules\selection\models\SrlmBcApplication5Search;
...
class ApplicationController extends Controller {
    ...
    public function actionIndex() {
        ...
        $searchModels = new SrlmBcApplication5Search();
    ...
    public function actionSingleapplication() {
        ...
        $district_option_model = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['district_code', 'district_name'])->distinct('district_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['and',
                    ['master_gram_panchayat_detail_bc.fifth_vacant' => 1],
                ])->all();
...
//        }
    ...
    public function actionValidateform($id) {
        ...
        $model = new \bc\modules\selection\modules\phasefive\models\form\ValidateByTliForm($bc_model);
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                ...
                if ($model->bc_model->save()) {
                    ...
                    $t = SrlmBcApplication5::find()->where(['form_number' => 6, 'form_data_validate' => 0])->one();
...
table#t01 th {
  ...
  color: white;
    ...
    public function actionDistrictgp($no = 0) {
        ...
        $district_query = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['district_code', 'district_name'])->distinct('district_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['master_gram_panchayat_detail_bc.fifth_vacant' => 1]);
        if ($no) {
            $district_query->andWhere(['master_gram_panchayat_detail_bc.fifth_complete' => 0]);
        }
    ...
    public function actionBlock($no = 0) {
        ...
        $district_query = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['block_code', 'block_name', 'district_name'])->distinct('block_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['master_gram_panchayat_detail_bc.fifth_vacant' => 1]);
        if ($no) {
            $district_query->andWhere(['master_gram_panchayat_detail_bc.fifth_complete' => 0]);
        }
    ...
    public function actionGp() {
        ...
        $dataProvider->query->andWhere(['master_gram_panchayat_detail_bc.fifth_vacant' => 1]);
        if ($searchModel->fifth_complete != '') {
            $dataProvider->query->andWhere(['master_gram_panchayat_detail_bc.fifth_complete' => $searchModel->fifth_complete]);
        }
...
table#t01 th {
  ...
  color: white;
    ...
    public function actionCsvgp() {
        ...
        if ($count <= \bc\modules\selection\models\base\GenralModel::MAX_ROW_DOWNLOAD_CSV) {
            ...
            foreach ($models as $model) {
                ...
                $application_complete = $model->gpdetail->fifth_complete;
    ...
    protected function findModel($id) {
        if (($model = SrlmBcApplication5::findOne($id)) !== null) {
            return $model;
        }

...
echo $form->field($model, 'fifth_complete')->label('Fifth Phase complete application')->widget(Select2::classname(), [
    'data' => range(0, 20),
...
<?= Html::button('<i class="fal fa-file-pdf"></i> Download PDF', ['class' => 'btn btn-primary btn-md', 'id' => 'download', 'name' => 'download', 'value' => 'download','style'=>'padding:7px 20px;']) . '&nbsp;' ?>
<?= Html::button('<i class="fal fa-file-excel"></i> Download CSV', ['class' => 'btn btn-primary btn-md', 'id' => 'downloadcsv', 'name' => 'downloadcsv', 'value' => 'downloadcsv','style'=>'padding:7px 20px;']) . '&nbsp;' ?>
<?php //} ?>
        </div>
...
$script = <<< JS
    ...
    $("#Searchform").attr({ "action":"/selection/phase5/application/gp"});
...
$js = <<<js
        ...
        $(document).ready(function(){
            $("#download").click(function(event){
              $("#Searchform").attr({ "action":"/selection/phase5/application/pdfgp"});
              $("#Searchform").removeAttr("data-pjax");
              $("#Searchform").submit();
            });
         $("#downloadcsv").click(function(event){
              $("#Searchform").attr({ "action":"/selection/phase5/application/csvgp"});
              $("#Searchform").removeAttr("data-pjax");
              $("#Searchform").submit();
            });

...
//        }
    ...
    public function actionValidateform($id) {
        ...
        if ($bc_model->form_data_validate != '0') {
            return $this->redirect(['/selection/phase3/application?DashboardSearchForm[form_data_validate]=0']);
        }

        $model = new \bc\modules\selection\modules\phasethree\models\form\ValidateByTliForm($bc_model);
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                ...
                if ($model->bc_model->save()) {
                    ...
                    $t = SrlmBcApplication3::find()->where(['form_number' => 6, 'form_data_validate' => 0])->one();

                    if (!empty($t)) {
                        return $this->redirect('/selection/phase3/application/view?id=' . $t->id);
                    } else {
                        return $this->redirect(['/selection/phase3/application?DashboardSearchForm[form_data_validate]=0']);
                    }
                }
...
table#t01 th {
  ...
  color: white;
    ...
    public function actionDistrictgp() {
        ...
        $district_model = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['district_code', 'district_name'])->distinct('district_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['master_gram_panchayat_detail_bc.third_vacant' => 1])->orderBy('district_name asc')->all();
    ...
    public function actionGp() {
        ...
        $dataProvider->query->andWhere(['master_gram_panchayat_detail_bc.third_vacant' => 1]);
        if ($searchModel->third_complete != '') {
            $dataProvider->query->andWhere(['master_gram_panchayat_detail_bc.third_complete' => $searchModel->third_complete]);
        }
...
table#t01 th {
  ...
  color: white;
    ...
    public function actionCsvgp() {
        ...
        if ($count <= \bc\modules\selection\models\base\GenralModel::MAX_ROW_DOWNLOAD_CSV) {
            ...
            foreach ($models as $model) {
                ...
                $application_complete = $model->gpdetail->third_complete;
    ...
    protected function findModel($id) {
        if (($model = SrlmBcApplication3::findOne($id)) !== null) {
            return $model;
        }

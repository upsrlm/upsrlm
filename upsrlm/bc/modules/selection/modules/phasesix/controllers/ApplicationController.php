...
namespace bc\modules\selection\modules\phasesix\controllers;
...
use bc\modules\selection\models\SrlmBcApplication6Search;
...
class ApplicationController extends Controller {
    ...
    public function actionIndex() {
        ...
        $searchModels = new SrlmBcApplication6Search();
    ...
    public function actionSingleapplication() {
        ...
        $district_option_model = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['district_code', 'district_name'])->distinct('district_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['and',
                    ['master_gram_panchayat_detail_bc.six_vacant' => 1],
                ])->all();
...
//        }
    ...
    public function actionValidateform($id) {
        ...
        if ($bc_model->form_data_validate != '0') {
            return $this->redirect(['/selection/phase6/application?DashboardSearchForm[form_data_validate]=0']);
        }

        $model = new \bc\modules\selection\modules\phasesix\models\form\ValidateByTliForm($bc_model);
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                ...
                if ($model->bc_model->save()) {
                    ...
                    $t = SrlmBcApplication6::find()->where(['form_number' => 6, 'form_data_validate' => 0])->one();

                    if (!empty($t)) {
                        return $this->redirect('/selection/phase5/application/view?id=' . $t->id);
                    } else {
                        return $this->redirect(['/selection/phase5/application?DashboardSearchForm[form_data_validate]=0']);
                    }
                }
            }
        }
...
table#t01 th {
  ...
  color: white;
    ...
    public function actionDistrictgp($no = 0) {
        ...
        $district_query = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['district_code', 'district_name'])->distinct('district_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['master_gram_panchayat_detail_bc.six_vacant' => 1]);
        if ($no) {
            $district_query->andWhere(['master_gram_panchayat_detail_bc.six_complete' => 0]);
        }
    ...
    public function actionBlock($no = 0) {
        ...
        $district_query = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['block_code', 'block_name', 'district_name'])->distinct('block_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['master_gram_panchayat_detail_bc.six_vacant' => 1]);
        if ($no) {
            $district_query->andWhere(['master_gram_panchayat_detail_bc.six_complete' => 0]);
        }
    ...
    public function actionGp() {
        ...
        $dataProvider->query->andWhere(['master_gram_panchayat_detail_bc.six_vacant' => 1]);
        if ($searchModel->six_complete != '') {
            $dataProvider->query->andWhere(['master_gram_panchayat_detail_bc.six_complete' => $searchModel->six_complete]);
        }
...
table#t01 th {
  ...
  color: white;
    ...
    protected function findModel($id) {
        if (($model = SrlmBcApplication6::findOne($id)) !== null) {
            return $model;
        }

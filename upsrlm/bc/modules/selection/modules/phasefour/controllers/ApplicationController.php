...
namespace bc\modules\selection\modules\phasefour\controllers;
...
use bc\modules\selection\models\SrlmBcApplication4Search;
...
class ApplicationController extends Controller {
    ...
    public function actionIndex() {
        ...
        $searchModels = new SrlmBcApplication4Search();
    ...
    public function actionListdownload() {
        ...
        foreach ($models as $model) {
            $model = SrlmBcApplication4::findOne($model['id']);
            $row = [
                $sr_no,
                $model->district_name,
                $model->block_name,
    ...
    public function actionSingleapplication() {
        ...
        $district_option_model = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['district_code', 'district_name'])->distinct('district_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['and',
                    ['master_gram_panchayat_detail_bc.fourth_vacant' => 1],
                ])->all();
...
//        }
    ...
    public function actionValidateform($id) {
        ...
        if ($bc_model->form_data_validate != '0') {
            return $this->redirect(['/selection/phase4/application?DashboardSearchForm[form_data_validate]=0']);
        }

        $model = new \bc\modules\selection\modules\phasefour\models\form\ValidateByTliForm($bc_model);
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                ...
                if ($model->bc_model->save()) {
                    ...
                    $t = SrlmBcApplication4::find()->where(['form_number' => 6, 'form_data_validate' => 0])->one();

                    if (!empty($t)) {
                        return $this->redirect('/selection/phase4/application/view?id=' . $t->id);
                    } else {
                        return $this->redirect(['/selection/phase4/application?DashboardSearchForm[form_data_validate]=0']);
                    }
                }
...
table#t01 th {
  ...
  color: white;
    ...
    public function actionDistrictgp() {
        ...
        $district_model = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['district_code', 'district_name'])->distinct('district_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['master_gram_panchayat_detail_bc.fourth_vacant' => 1])->orderBy('district_name asc')->all();
    ...
    public function actionGp() {
        ...
        $dataProvider->query->andWhere(['master_gram_panchayat_detail_bc.fourth_vacant' => 1]);
        if ($searchModel->fourth_complete != '') {
            $dataProvider->query->andWhere(['master_gram_panchayat_detail_bc.fourth_complete' => $searchModel->fourth_complete]);
        }
...
table#t01 th {
  ...
  color: white;
    ...
    public function actionCsvgp() {
        ...
        if ($count <= \bc\modules\selection\models\base\GenralModel::MAX_ROW_DOWNLOAD_CSV) {
            ...
            foreach ($models as $model) {
                ...
                $application_complete = $model->gpdetail->fourth_complete;
    ...
    protected function findModel($id) {
        if (($model = SrlmBcApplication4::findOne($id)) !== null) {
            return $model;
        }

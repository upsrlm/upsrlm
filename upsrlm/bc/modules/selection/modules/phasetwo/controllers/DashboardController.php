...
class DashboardController extends Controller {
    ...
    public function actionIndex() {
        ...
        $registeruser = \bc\modules\selection\models\SrlmBcSelectionUser::find()->select(['id'])->where(['phase' => 2])->count();
        $district_option_model = \bc\models\master\MasterGramPanchayat::find()->joinWith(['gpdetail'])->select(['district_code', 'district_name'])->distinct('district_code')->andWhere(['master_gram_panchayat.status' => 1])->andWhere(['or',
                    ['master_gram_panchayat_detail_bc.second_vacant' => 1],
    ...
    public function actionDownload() {
        ...
        foreach ($models as $model) {
            ...
            $row = [
                ...
                $model->form_number,
                $form_start_date,
                SrlmBcApplication2::find()->select('id')->where(['mobile_number' => $model->mobile_number, 'form_number' => 6])->asArray()->count(),
                SrlmBcApplication2::find()->select('id')->where(['aadhar_number' => $model->aadhar_number, 'form_number' => 6])->asArray()->count(),

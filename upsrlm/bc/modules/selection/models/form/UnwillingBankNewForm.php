...
class UnwillingBankNewForm extends Model {
...
//        }
        ...
        if ($this->unwilling_model->validate()) {
            if ($this->unwilling_model->save()) {
                $this->bc_model->bc_unwilling_bank = 1;
                $this->bc_model->bc_unwilling_bank_by = \Yii::$app->user->identity->id;
                $this->bc_model->bc_unwilling_bank_date = new Expression('NOW()');
                $this->bc_model->action_type = SrlmBcApplication::ACTION_TYPE_BANK_UNWILLING;
                $this->bc_model->save();
                $user_model = \common\models\User::findOne(['id' => $this->bc_model->user_id]);
                if ($user_model != null) {
